...
def process_workbook(stream, week: int) -> BytesIO:
    ...
    # Rewrite Cash Funding
    cf.delete_rows(1, cf.max_row)
    for r in dataframe_to_rows(df_cf, index=False, header=True): cf.append(r)
    ...
    # Rewrite Report Batch
    rb.delete_rows(1, rb.max_row)
    for r in dataframe_to_rows(df_rb, index=False, header=True): rb.append(r)
    tbl_rb = Table(displayName=f"ReportBatchTable_Week{week}", ref=f"A1:{get_column_letter(rb.max_column)}{rb.max_row}")
    ...
    for col in ['I','J','K','L']:
        for cell in cf[col][1:]: cell.number_format = '$#,##0.00'
